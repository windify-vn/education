...
def sync_homework_to_topic(doc, method=None):
	"""
	Sync Homework to Topic's custom_homework child table when Homework is saved.
	Also auto-fill course from student_group.
	Business rule: 1 Homework can only belong to 1 Topic.
	When Topic changes, remove from old Topic and add to new Topic.
	"""
	# Auto-fill course from student_group
	if doc.get('student_group') and not doc.get('course'):
		course = frappe.db.get_value('Student Group', doc.student_group, 'course')
		if course:
			doc.course = course
			frappe.db.set_value('Homework', doc.name, 'course', course, update_modified=False)
	
	linked_topic = doc.get('linked_topic')
	
	# First, remove this homework from ALL other Topics (ensure 1:1 relationship)
	# Find all existing records for this homework
	...
	# Remove from old Topics (if linked_topic changed or cleared)
	for record in existing_records:
		if record.parent != linked_topic:
			# Delete the old record
			frappe.db.delete('Topic Homework', {'name': record.name})
	
	# If no linked_topic, we're done (homework unlinked from all topics)
	if not linked_topic:
		frappe.db.commit()
		return
	
	# Check if this homework already exists in the NEW Topic
	existing_in_new_topic = frappe.db.exists(
		'Topic Homework',
		{
			'parent': linked_topic,
			'parenttype': 'Topic',
	...
	if existing_in_new_topic:
		return
	
	# Add homework to Topic's custom_homework child table
	topic = frappe.get_doc('Topic', linked_topic)
	
	# Check if custom_homework field exists
	if not hasattr(topic, 'custom_homework'):
		return
	
	# Add new row to custom_homework
	topic.append('custom_homework', {
		'homework': doc.name
	})
...
def validate_homework(doc, method=None):
	"""
	Validate and auto-fill fields for Homework before saving.
	- Auto-fill course from student_group
	"""
	# Auto-fill course from student_group
