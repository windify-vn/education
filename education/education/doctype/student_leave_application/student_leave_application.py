...
class StudentLeaveApplication(Document):
	...
	def validate_duplicate(self):
		# Build filter based on attendance_based_on
		filters = {
		...
		# Add student_group or course_schedule filter to allow same student 
		# to have leave for different groups on same date
		if self.attendance_based_on == "Student Group" and self.student_group:
			data = frappe.db.sql(
				...
				where
					...
					name != %(name)s and student = %(student)s and 
					student_group = %(student_group)s and docstatus < 2
			""",
				{**filters, "student_group": self.student_group},
				as_dict=1,
			)
		elif self.attendance_based_on == "Course Schedule" and self.course_schedule:
			data = frappe.db.sql(
				"""select name from `tabStudent Leave Application`
				where
					((%(from_date)s > from_date and %(from_date)s < to_date) or
					...
					(%(from_date)s <= from_date and %(to_date)s >= to_date)) and
					name != %(name)s and student = %(student)s and 
					course_schedule = %(course_schedule)s and docstatus < 2
			""",
				{**filters, "course_schedule": self.course_schedule},
				as_dict=1,
			)
		else:
			# Fallback to original behavior (check by student + date only)
			data = frappe.db.sql(
				"""select name from `tabStudent Leave Application`
				where
					((%(from_date)s > from_date and %(from_date)s < to_date) or
	...
	def update_attendance(self):
		...
		for dt in daterange(getdate(self.from_date), getdate(self.to_date)):
			...
			# Build filter based on attendance_based_on to check attendance per group/course
			attendance_filter = {
				"student": self.student, 
				"date": date, 
				"docstatus": ("!=", 2)
			...
			# Add student_group or course_schedule filter to allow same student 
			# to have attendance for different groups on same date
			if self.attendance_based_on == "Student Group" and self.student_group:
				attendance_filter["student_group"] = self.student_group
			elif self.attendance_based_on == "Course Schedule" and self.course_schedule:
				attendance_filter["course_schedule"] = self.course_schedule
	...
	def cancel_attendance(self):
		if self.docstatus == 2:
			# Build query based on attendance_based_on to cancel only attendance for this group/course
			if self.attendance_based_on == "Student Group" and self.student_group:
				attendance = frappe.db.sql(
					...
					WHERE
						...
						docstatus < 2
				""",
					(self.student, self.student_group, self.from_date, self.to_date),
					as_dict=1,
				)
			elif self.attendance_based_on == "Course Schedule" and self.course_schedule:
				attendance = frappe.db.sql(
					...
					WHERE
						student = %s and
						course_schedule = %s and
						(date between %s and %s) and
						docstatus < 2
				""",
					(self.student, self.course_schedule, self.from_date, self.to_date),
					as_dict=1,
				)
			else:
